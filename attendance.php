...
<head>
    ...
    <title>Event Insight | Attendance Management</title>
					...
					<div class="table-wrapper">
						<div class="table-title">
							<div class="row">
								...
								<div class="col-sm-6">
								<a href='report.php' class="btn btn-success"><i class="material-icons">&#xe5c4;</i><span>Back</span></a>
					...
					<table class="table">
						<thead>
							<tr>
							    <th>Event Title</th>
								<th>Meeting Title</th>
								<th>Attendees Full Name</th>
								<th>Date Created</th>
								<th>Status</th>
								<th>If you choose Absent, attach document</th>
								<th>Absent Comment</th>
								<th>Actions</th>
							</tr>
						</thead>
						<tbody>
                        ...
                        $sql = "SELECT * FROM attendance LIMIT $offset, $limit";
                        ...
                        if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                ...
                                echo "<td>" . $row["attendeesname"] . "</td>";
                                echo "<td>" . $row["attendance_datecreated"] . "</td>";
                                echo "<td>" . $row["attendance_status"] . "</td>";
                                // Check if mc_letter is set
                                if (!empty($row["mc_letter"])) {
                                    // Assume mc_letter is a Google Drive link
                                    echo "<td><a href='" . $row["mc_letter"] . "' target='_blank'>View</a></td>";
                                } else {
                                    echo "<td>No link available</td>";
                                }
                        
                                echo "<td>" . $row["provencomment"] . "</td>";
                                echo "<td>
                                        <a href='attendanceedit.php?edit=" . $row["attendance_id"] . "'><i class='material-icons'>edit</i></a>
                                        <a href='attendancedelete.php?del=" . $row["attendance_id"] . "'><i class='material-icons'>delete</i></a>
                                    </td>";
                                echo "</tr>";
                            }
                        } else {
                            echo "<tr><td colspan='8'>No attendance found</td></tr>";
                        }
