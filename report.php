...
<head>
    ...
    <title>Event Insight | Report Management Analysis</title>
			...
			<div class="recentOrders">
				<div class="table-responsive">
					<div class="table-wrapper">
						<div class="table-title">
							<div class="row">
								...
								<div class="col-sm-6">
								<a href="generate_final_report.php" class="btn btn-primary" target="_blank"><i class="material-icons">&#xf090;</i> <span>Download Report</span></a>
								<a href="#" class="btn btn-success" onclick="location.href='/syncmastercalendar/reportform.php';"><i class="material-icons">&#xe146;</i> <span>Add Report</span></a>
					...
					<table class="table">
						...
						<tbody>
							...
							$countQuery = "SELECT COUNT(*) as total FROM report";
							$countResult = $conn->query($countQuery);
							$totalEntries = $countResult->fetch_assoc()['total'];

							$sql = "SELECT * FROM report LIMIT $offset, $limit";
                            ...
                            while ($row = $result->fetch_assoc()) {
                                ...
                                // Fetch and display event_title based on event_id
                                $eventId = $row["event_id"];
                                $eventQuery = "SELECT title FROM events WHERE event_id = $eventId";
                                ...
                                if (!empty($row["picture_url"])) {
                                    echo "<td><a href='" . $row["picture_url"] . "' target='_blank'>View</a></td>";
                                } else {
                                    echo "<td>No link available</td>";
                                }
                                
                                if (!empty($row["file_url"])) {
                                    echo "<td><a href='" . $row["file_url"] . "' target='_blank'>Download</a></td>";
                                } else {
                                    echo "<td>No link available</td>";
                                }
                                ...
                                echo "<td>
                                        <a href='reportedit.php?edit=" . $row["report_id"] . "'><i class='material-icons'>edit</i></a>
                                        <a href='reportdelete.php?del=" . $row["report_id"] . "'><i class='material-icons'>delete</i></a>
                                    </td>";
                                echo "</tr>";
                            }
                        } else {
				...
				<ul class="pagination" id="pagination">
					...
					for ($i = 1; $i <= ceil($result->num_rows / $limit); $i++) {
						echo "<li class='page-item'><a href='report.php?page=$i' class='page-link' onclick='changePage($i)'>$i</a></li>";
					}
					?>
			...
			<script>
				...
				function changePage(newPage) {
					if (newPage >= 1 && newPage <= totalPages) {
						...
						window.location.href = 'report.php?page=' + currentPage;
					}
				}
